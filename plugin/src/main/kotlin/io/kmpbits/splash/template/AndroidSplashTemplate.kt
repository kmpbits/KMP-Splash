...
internal object AndroidSplashTemplate {
     ...
     * [postSplashTheme] overrides `postSplashScreenTheme` with a consumer-supplied theme
     * reference (e.g. `"@style/Theme.MyApp"`) instead of generating the fallback `Theme.App`.
     * Needed for `AppCompatActivity`, which requires an AppCompat-descended theme to be active
     * by `setContentView()` time — see the README's "Using a Different Base Class" section.
     */
    fun generateThemes(
        backgroundColor: String,
        logoDrawableName: String? = null,
        postSplashTheme: String? = null,
    ): String {
        val logoItem = if (logoDrawableName != null) {
            """        <item name="windowSplashScreenAnimatedIcon">@drawable/$logoDrawableName</item>"""
        } else ""

        val postSplashThemeRef = postSplashTheme ?: "@style/Theme.App"
        val fallbackThemeBlock = if (postSplashTheme == null) {
...
$logoItem
        <item name="postSplashScreenTheme">$postSplashThemeRef</item>
    </style>$fallbackThemeBlock
</resources>"""
    }
     ...
     * [postSplashTheme] behaves the same as in [generateThemes].
     */
