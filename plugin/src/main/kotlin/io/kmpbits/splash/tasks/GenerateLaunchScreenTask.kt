...
abstract class GenerateLaunchScreenTask : DefaultTask() {
        ...
        val lightColorJson = """
    {
      ...
      "idiom": "universal"
        ...
        val darkColorJson = if (nightColor != null) {
            val (dr, dg, db) = parseHexColor(nightColor)
            """,
    {
      "appearances": [ { "appearance": "luminosity", "value": "dark" } ],
      "color": {
        "color-space": "srgb",
        "components": { "alpha": "1.000", "red": "$dr", "green": "$dg", "blue": "$db" }
      },
      "idiom": "universal"
    }"""
        } else ""
...
import androidx.compose.ui.graphics.painter.Painter""" else ""
        ...
        val bgColorLine = if (nightArgb != null) {
            "SplashDefaults.backgroundColor = if (isSystemInDarkTheme()) Color($nightArgb) else Color($lightArgb)"
        } else {
            "SplashDefaults.backgroundColor = Color($lightArgb)"
        }
