...
### Added
- **`uiFramework = UiFramework.Native` — splash support for KMP apps with a SwiftUI iOS frontend.** Until now the plugin assumed Compose Multiplatform on iOS: it generated a `SplashInit.kt` feeding the `SplashConfig` composable. Set `splashScreen { uiFramework = UiFramework.Native }` and the plugin instead generates a `KmpSplashView.swift` straight into your Xcode project (`iosProjectPath`) and wires it into the build — no `io.github.kmpbits:splash-runtime` dependency required. Wrap your root view in `KmpSplashView(awaitReady:) { ... }`; `awaitReady` is an optional Swift `async` closure (omit it to just hold the launch screen until the first SwiftUI frame, then run the configured `exitAnimation`). Native asset generation (`UILaunchScreen`, `Assets.xcassets`, app icon) is unchanged. For Xcode 16+ projects (synchronized folder groups) no `project.pbxproj` change is needed; for older projects the plugin patches the Sources build phase automatically. Defaults to `UiFramework.Compose` — existing projects are unaffected. See the README's "SwiftUI (native iOS UI)" section.

### Fixed
- **`KmpSplashView`'s logo no longer shifts down when the native launch screen hands off to it.** `UILaunchScreen` centers its image against the full screen bounds, ignoring the safe area entirely (it predates the safe-area API). The generated `Image` didn't have `.ignoresSafeArea()`, so SwiftUI centered it within the safe area instead — visibly lower on any device where the top inset (Dynamic Island/notch) is bigger than the bottom one (home indicator), i.e. most current iPhones. `.ignoresSafeArea()` now applies to the whole splash `ZStack`, not just the background color.
...
- **`androidAppPath` mode now wires the generated `SplashInit.kt` into the `androidApp` module, not the module applying the plugin.** It always imports `androidx.compose.*`, but was being added to the applying project's own `androidMain` regardless of `androidAppPath` — an `Unresolved reference 'compose'` compile error on any project where that module (often a shared/business-logic module) doesn't depend on Compose, which is the norm once iOS moves to `UiFramework.Native`. It's now routed into the `androidAppPath` module's own Kotlin source set instead — matching whichever of the two shapes that module actually is: a Kotlin Multiplatform module with `androidTarget()` (its `androidMain` source set — AGP's own `sourceSets.main.kotlin` is not read by a KMP Android target at all, so wiring only through AGP silently compiled nothing) or a plain `org.jetbrains.kotlin.android` module like this plugin's own `sample/androidApp` (AGP's `sourceSets.main.kotlin`, which is where that plugin publishes Kotlin sources). The manifest and resource wiring were already correctly routed to `androidAppPath`'s module — a missing class in an otherwise-correct manifest is exactly what produced `Unable to get provider io.kmpbits.splash.KmpSplashInitProvider` at app startup. `androidAppPath` is now a required field when `uiFramework = UiFramework.Native` and the project has an Android target — the plugin fails fast with an actionable message instead of producing either error.
...
- **`splash-runtime`'s `androidx.core:core-splashscreen` dependency is now `api` instead of `implementation`.** `installKmpSplash()` returns `SplashScreen`, a type from that dependency; with it scoped as `implementation`, consumers that don't already depend on `core-splashscreen` directly couldn't compile a call to `installKmpSplash()` ("Cannot access class 'androidx.core.splashscreen.SplashScreen'"). Same class of fix as `androidx.activity` being made `api` in 1.2.0 for `SplashActivity`.
